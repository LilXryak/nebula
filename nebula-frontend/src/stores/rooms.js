...
export const useRoomsStore = defineStore('rooms', () => {
  ...
  const loadHistory = () => {
    try {
      const saved = localStorage.getItem('nebula_room_history')
      if (saved) {
  ...
  const saveHistoryToStorage = () => {
    try {
      localStorage.setItem('nebula_room_history', JSON.stringify(roomHistory.value))
    } catch (error) {
  ...
  const clearHistory = () => {
    roomHistory.value = []
    try {
      localStorage.removeItem('nebula_room_history')
    } catch (error) {
