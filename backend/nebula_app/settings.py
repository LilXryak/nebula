# nebula_app/settings.py - Django main settings configuration
import os
...
WSGI_APPLICATION = 'nebula_app.wsgi.application'
ASGI_APPLICATION = 'nebula_app.asgi.application'

# Database configuration
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': config('DB_NAME', default='nebula_db'),
...
if not DEBUG:
    # Add your production domains from config
    ...
    # Also add from CORS_ALLOWED_ORIGINS if specified
...
# CORS settings - автоматически настроить из CORS_ALLOWED_ORIGINS или DOMAIN_NAME
cors_default = 'http://localhost:3000,http://127.0.0.1:3000'
if not DEBUG:
    domain_name = config('DOMAIN_NAME', default='')
    if domain_name:
        cors_default = f'https://{domain_name},https://www.{domain_name}'
        if config('CORS_ALLOWED_ORIGINS', default=''):
            cors_default = config('CORS_ALLOWED_ORIGINS', default=cors_default)

CORS_ALLOWED_ORIGINS = config('CORS_ALLOWED_ORIGINS', default=cors_default).split(',')
...
# В файле backend/nebula_app/settings.py замените секцию LOGGING на:
